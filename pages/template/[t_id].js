...
const CustomTemplate = () => {
  ...
  const [insertRecord] = useMutation(INSERT_RECORD, {
    onCompleted(proxy, result) {
      if (proxy.insertDataFromField.msg.message) {
        handleOpration("success", proxy.insertDataFromField.msg.message);
      } else {
        handleOpration("error", proxy.insertDataFromField.msg.error);
      }
    },
    onError(err) {
  ...
  const [updateRecord] = useMutation(UPDATE_RECORD, {
    onCompleted(proxy, result) {
      if (proxy.updateDataFromField.msg.message) {
        handleOpration("success", proxy.updateDataFromField.msg.message);
      } else {
        handleOpration("error", proxy.updateDataFromField.msg.error);
      }
    },
    onError(err) {
      handleOpration("error", err.message);
  ...
  const [deleteRecord] = useMutation(DELETE_RECORD, {
    onCompleted(proxy, result) {
      if (proxy.deleteDataFromField.msg.message) {
        handleOpration("success", proxy.deleteDataFromField.msg.message);
      } else {
        handleOpration("error", proxy.deleteDataFromField.msg.error);
      }
    },
    onError(err) {
      handleOpration("error", err.message);
        ...
        {!loadingForm && fieldsList.length > 0 ? (
          <>
            <Box className={style.frmTemplate}>
              <form className={style.formData} ref={ref}>
                        ...
                        <FormControl style={{ width: 220 }}>
                          ...
                          >
                            {input.pickList.map((item, index) => {
                              return (
                                <MenuItem key={`mm_${index}`} value={item}>
                                  {item}
                                </MenuItem>
                              );
                            })}
